...
def name_surface_file(args, dir_file):
    ...
    # resolution
    surf_file += '/[%s,%s,%d]' % (str(args.xmin), str(args.xmax), int(args.xnum))
    if args.y:
    ...
    with h5py.File(surf_file, 'r+' if rank == 0 else 'r') as f:
        ...
        for count, ind in enumerate(inds):
            ...
            if args.dataset in ['cifar10', 'imagenet']:
                ...
                loss = trainer.test(net, verbose=False)[0]['mse_test']
                acc = 0
...
if __name__ == '__main__':
    ...
    # --------------------------------------------------------------------------
    crunch(surf_file, net, w, s, d, testloader, 'train_loss', 'train_acc', comm, rank, args)
